...
#else
	static mavlink_status_t m_mavlink_status[MAVLINK_COMM_NUM_BUFFERS];   //串口通道状态
#endif
...
MAVLINK_HELPER mavlink_message_t* mavlink_get_channel_buffer(uint8_t chan)
{
	...
	// has to be defined externally
#else
	static mavlink_message_t m_mavlink_buffer[MAVLINK_COMM_NUM_BUFFERS];  //串口缓冲区单帧数据
#endif
					    ...
					    const mavlink_message_t *msg)
{
	const uint8_t *p ;
	const uint8_t *psig ;
	const uint8_t *incoming_signature;
	mavlink_sha256_ctx ctx;
	uint8_t signature[6];
	uint16_t i;
	uint8_t link_id;
  ...
  p = (const uint8_t *)&msg->magic;
	psig = msg->signature;
  incoming_signature = psig+7;
...
#define MAVLINK_MESSAGE_LENGTH(msgid) mavlink_message_lengths[msgid]
	...
	switch (status->parse_state)   //上一次解析数据完成的状态
	{
	case MAVLINK_PARSE_STATE_UNINIT:
	case MAVLINK_PARSE_STATE_IDLE:     //空闲状态
		if (c == MAVLINK_STX)
		{
			status->parse_state = MAVLINK_PARSE_STATE_GOT_STX;   //已经识别到数据起始位
			rxmsg->len = 0;																			 //有效载荷赋值为0
			rxmsg->magic = c;                                    //赋值起始位
...
/* Support shorter buffers than the
   default maximum packet size */
				...
				|| c > MAVLINK_MAX_PAYLOAD_LEN    //数据长度错误
#endif
				)
		{
		...
		else
		{
			...
			mavlink_update_checksum(rxmsg, c);
                        if (status->flags & MAVLINK_STATUS_FLAG_IN_MAVLINK1) {
                            ...
                            status->parse_state = MAVLINK_PARSE_STATE_GOT_LENGTH;     //正确解析有效载荷的长度
                        }
		}
		break;
	...
	case MAVLINK_PARSE_STATE_GOT_MSGID3:
		_MAV_PAYLOAD_NON_CONST(rxmsg)[status->packet_idx++] = (char)c;  //存储有效载荷数据
		mavlink_update_checksum(rxmsg, c);
	...
	case MAVLINK_PARSE_STATE_GOT_PAYLOAD: {
		...
		if (c != (rxmsg->checksum & 0xFF)) {    //验证校验位
			status->parse_state = MAVLINK_PARSE_STATE_GOT_BAD_CRC1;    //低位错误
		} else {
	...
	case MAVLINK_PARSE_STATE_GOT_BAD_CRC1:
		if (status->parse_state == MAVLINK_PARSE_STATE_GOT_BAD_CRC1 || c != (rxmsg->checksum >> 8)) {
			...
			status->msg_received = MAVLINK_FRAMING_BAD_CRC;   //CRC校验错误，数据无效
		} else {
			// Successfully got message
			status->msg_received = MAVLINK_FRAMING_OK;       //校验正确，返回解析成功
		}
		rxmsg->ck[1] = c;
	...
	if (status->msg_received == MAVLINK_FRAMING_OK)
	{
		...
		status->packet_rx_success_count++;     //成功接收到的数据帧的数量
	}
