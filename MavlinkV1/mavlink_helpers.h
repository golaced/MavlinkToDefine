...
#else
	static mavlink_status_t m_mavlink_status[MAVLINK_COMM_NUM_BUFFERS]; /*串口通道状态标志数据，结构体*/
#endif
...
#endif
{
	...
	mavlink_get_channel_status(chan)->current_tx_seq = mavlink_get_channel_status(chan)->current_tx_seq+1; /*发送帧计数，最大值255*/
	msg->checksum = crc_calculate(((const uint8_t*)(msg)) + 3, MAVLINK_CORE_HEADER_LEN); /*帧头CRC校验，不包含帧头， MAVLINK_CORE_HEADER_LEN=5*/
...
/**
 * @brief Finalize a MAVLink message with MAVLINK_COMM_1 as default channel
 */
#if MAVLINK_CRC_EXTRA
MAVLINK_HELPER uint16_t mavlink_finalize_message(mavlink_message_t* msg, uint8_t system_id, uint8_t component_id, 
						 uint8_t min_length, uint8_t length, uint8_t crc_extra)
{
    return mavlink_finalize_message_chan(msg, system_id, component_id, MAVLINK_COMM_1, min_length, length, crc_extra);
}
#else
MAVLINK_HELPER uint16_t mavlink_finalize_message(mavlink_message_t* msg, uint8_t system_id, uint8_t component_id, 
						 uint8_t length)
{
	return mavlink_finalize_message_chan(msg, system_id, component_id, MAVLINK_COMM_1, length);
}
#endif
...
#endif
{
	...
	buf[3] = gcs_mavlink_system.sysid;
	buf[4] = gcs_mavlink_system.compid;
	buf[5] = msgid;
...
#define MAVLINK_MESSAGE_LENGTH(msgid) mavlink_message_lengths[msgid]
	...
	status->msg_received = MAVLINK_FRAMING_INCOMPLETE;      //解析标志位，解析中...  

	switch (status->parse_state)    //具体解析到数据帧的哪一部分   
	{
		/*	STX	*/
	case MAVLINK_PARSE_STATE_UNINIT:
	case MAVLINK_PARSE_STATE_IDLE:		//没有获得起始位之前，位空闲状态
		if (c == MAVLINK_STX)
		{
			status->parse_state = MAVLINK_PARSE_STATE_GOT_STX;		//获取到起始位
			rxmsg->len = 0;
			rxmsg->magic = c;																			//赋值起始位
...
/* Support shorter buffers than the
   default maximum packet size */
				...
				|| c > MAVLINK_MAX_PAYLOAD_LEN   			//判断包长是否合理
#endif
				)
		{
	...
	case MAVLINK_PARSE_STATE_GOT_COMPID:
		...
		if (rxmsg->len == 0)
		{
			status->parse_state = MAVLINK_PARSE_STATE_GOT_PAYLOAD;   //如果长度为0，重新获取 ，这里没有看懂
		}
		else
		{
	...
	case MAVLINK_PARSE_STATE_GOT_MSGID:
		_MAV_PAYLOAD_NON_CONST(rxmsg)[status->packet_idx++] = (char)c;   //存储一个字节数据
		mavlink_update_checksum(rxmsg, c);
...
#endif
		if (c != (rxmsg->checksum & 0xFF)) {    //直接接受到的校验和低8位与运算得到后的低8位进行比较
			status->parse_state = MAVLINK_PARSE_STATE_GOT_BAD_CRC1;    //低8 错误
		} else {
			status->parse_state = MAVLINK_PARSE_STATE_GOT_CRC1;        //低8 正确
		}
                _MAV_PAYLOAD_NON_CONST(rxmsg)[status->packet_idx] = (char)c;   //将crc1 赋值到消息帧中
		break;
	...
	case MAVLINK_PARSE_STATE_GOT_BAD_CRC1:
		if (status->parse_state == MAVLINK_PARSE_STATE_GOT_BAD_CRC1 || c != (rxmsg->checksum >> 8)) {
			...
			status->msg_received = MAVLINK_FRAMING_BAD_CRC;  //校验错误
		} else {
			...
			status->msg_received = MAVLINK_FRAMING_OK;
                ...
                memcpy(r_message, rxmsg, sizeof(mavlink_message_t));    //将接收到消息帧复制到r_message中
		break;
	}
  ...
  r_mavlink_status->packet_rx_drop_count = status->parse_error;//  r_mavlink_status->packet_rx_drop_count += status->parse_error;//	; 这里源码应该有错误   //错误的个数
...
void comm_send_ch(mavlink_channel_t chan, uint8_t ch)  //add by Awesome
{
	switch(chan)
	{
		case MAVLINK_COMM_1:
