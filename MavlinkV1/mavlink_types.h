...
#pragma anon_unions //add by Awesome
...
//  #define MAVPACKED( __Declaration__ ) __pragma( pack(push, 1) ) __Declaration__ __pragma( pack(pop) )
	#define MAVPACKED( __Declaration__ ) __Declaration__  //modify by Awesome
#endif
...
#else
# define MAVLINK_COMM_NUM_BUFFERS 4    //嵌入式串口通道
#endif
#endif
...
typedef struct __mavlink_status {
    uint8_t msg_received;               ///< Number of received messages    //解析状态，对应枚举mavlink_framing_t
    uint8_t buffer_overrun;             ///< Number of buffer overruns      //已经超过包允许的最大长度的次数
		uint8_t parse_error;                ///< Number of parse errors					//解析错误次数(关于这个标志位的代码似乎有点问题)
    mavlink_parse_state_t parse_state;  ///< Parsing state machine					//解析到数据位的位置，对应枚举mavlink_parse_state_t
    uint8_t packet_idx;                 ///< Index in current packet				//当前有效字节的位置（个数）
    uint8_t current_rx_seq;             ///< Sequence number of last packet received   //将要接收到的消息数字序号
    uint8_t current_tx_seq;             ///< Sequence number of last packet sent       //将要发送的消息的数字序号
    uint16_t packet_rx_success_count;   ///< Received packets								//解析成功次数
    uint16_t packet_rx_drop_count;      ///< Number of packet drops					//解析错误次数
} mavlink_status_t;
